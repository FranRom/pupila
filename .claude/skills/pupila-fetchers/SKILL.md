...
The pipeline ingests from 18 public sources (4 ATS APIs — Ashby, Greenhouse, Lever, Recruitee — plus RSS, JSON boards, HN, HTML scrapers, an Aave Next.js scraper, `ashby-private` for orgs whose public posting-API is disabled, `jobicy` and `himalayas` — no-key remote-jobs feeds, and `bluedoor` — a free cross-ATS aggregator queried by region from `profile.location`). Adding a source means: a fetcher, a normalizer, a `Source` literal, a slot in the orchestrator, and dedup/render wiring.
...
`aave = ashby-private > ashby > lever > greenhouse > recruitee > cryptojobslist > web3career > aijobsnet > hn-hiring > hn-jobs > remotive > weworkremotely > remoteok > jobicy > himalayas > remoteyeah > bluedoor`
