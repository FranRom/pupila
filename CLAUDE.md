...
The repo ships **neutral defaults** in `config/profile.json` and `config/slugs.json` so any forker can plug in their own role/sector/region. The original frontend + web3 + AI configuration this repo was built from is preserved at `examples/franron/` as a worked reference.
...
examples/
  franron/          # the original frontend + web3 + AI worked example (profile.json, slugs.json, candidate-brief.md, applied.json)
    README.md         # how the example was tuned and how to copy bits into your own config
