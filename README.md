...
- **9-panel Settings dashboard**: switch LLM CLI / install or remove the scheduler / regenerate scoring profile / manage job sources (incl. LLM-powered company discovery) / inspect the last run / check disk usage / clean / view environment / monitor the apply queue, all from `pnpm run ui` → Settings.
